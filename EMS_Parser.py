...
def status_parse(region, date, worksheet, rtus):
    ...
    archive_dir_dest = 'Z:\\Clients\\TND\\FirstEnr\\82568_EtfScadaSupprt\\Design\\Substation Projects\\EMS MODEL SCREEN DUMPS\\' + region + '\\' + 'ALBURTIS' + '\\' + 'Archive' + '\\'
    if not os.path.exists(archive_dir_dest):
        os.makedirs(archive_dir_dest)

    outfile_dir = 'Z:\\Clients\\TND\\FirstEnr\\82568_EtfScadaSupprt\\Design\\Substation Projects\\EMS MODEL SCREEN DUMPS\\' + region + '\\' + 'ALBURTIS' + '\\'

    # if possible, move all old dump worksheets from inside rtu folder to the archive folder. we want to keep them, but it looks cluttered
...
#        archive_dir_dest = 'Z:\\Clients\\TND\\FirstEnr\\82568_EtfScadaSupprt\\Design\\Substation Projects\\EMS MODEL SCREEN DUMPS\\' + region + '\\' + rtu + '\\' + 'Archive' + '\\'
#        if not os.path.exists(archive_dir_dest):
#            os.makedirs(archive_dir_dest)
